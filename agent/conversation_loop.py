                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                agent._vprint(f"{agent.log_prefix}   💡 Hermes' Codex OAuth login was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      run `hermes auth add openai-codex` to create a fresh Hermes-owned login.", force=True)
